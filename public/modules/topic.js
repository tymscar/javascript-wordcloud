class Topic {
	...
	intersects(other) {
		if (
			this.left >= other.left + other.width + other.padding ||
			other.left >= this.left + this.width + this.padding
		)
			return false;
		if (
			this.down >= other.down + other.height + other.padding ||
			other.down >= this.down + this.height + this.padding
		)
			return false;
		return true;
	}
